            ...
            <div className="mx-auto grid w-full max-w-[42rem] gap-3 sm:gap-4 xl:max-w-none xl:grid-cols-[4.5rem_minmax(0,1fr)]">
              <div className="order-2 flex gap-3 xl:order-1 xl:flex-col">
                {Array.from({ length: 4 }).map((_, index) => (
                  <Skeleton key={index} className="h-16 w-16 rounded-2xl sm:h-20 sm:w-20 xl:h-[4.5rem] xl:w-[4.5rem]" />
                ))}
              </div>
              <Skeleton className="order-1 aspect-square rounded-[2rem] xl:order-2" />
            </div>
