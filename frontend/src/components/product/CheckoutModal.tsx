...
export const CheckoutModal = memo(function CheckoutModal() {
  ...
  const [form, setForm] = useState<DeliveryFormValues>(() => createEmptyDeliveryForm());
  ...
  const applySuggestedCoupon = useCallback(
    (code: string) => {
      setCouponCode(code);
  ...
  const update = useCallback(
    (k: keyof DeliveryFormValues) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
            ...
            {done ? (
              ...
              <>
                ...
                <form onSubmit={submit} className="mt-6 space-y-4">
                  <DeliveryDetailsFields form={form} onChange={update} dense />
