...
export const getPerfumeRequests = asyncHandler(async (req, res) => {
  ...
  const [perfumeRequests, total] = await Promise.all([
    PerfumeRequest.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean({ virtuals: true }),
    PerfumeRequest.countDocuments(filter),
  ]);

  res.json({
    success: true,
    data: {
      requests: perfumeRequests.map(normalizePerfumeRequestResponse),
